...
<Callout type="info">
  The MongoDB adapter currently provides core authentication features. RBAC and
  refresh token features are planned for future releases.
</Callout>
...
db.keyloom_Account.createIndex(
...
// Verification tokens
db.keyloom_VerificationToken.createIndex(
  { identifier: 1, tokenHash: 1 },
  { unique: true }
);
db.keyloom_VerificationToken.createIndex(
  { expiresAt: 1 },
...
<Callout>
  For MongoDB Atlas users, enable connection pooling and use read preferences
