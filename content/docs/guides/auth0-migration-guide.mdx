...
<Callout type="info">
Migration typically takes 2-4 hours for most applications. Plan for a maintenance window if you need zero downtime.
</Callout>
...
function transformAuth0User(auth0User) {
  return {
    ...
    accounts: auth0User.identities?.map(identity => ({
