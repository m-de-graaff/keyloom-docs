...
| API                         | From                           | Signature                                                                                                             | Returns                 |
| --------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------- | ----------------------- | --------------------- |
| `toPermissionMap(rbac)`     | `@keyloom/core/rbac/policy`    | `(RbacConfig) => Record<permission, role[]>`                                                                          | permission to roles map |
| `withRole(handler, opts)`   | `@keyloom/core/rbac/with-role` | `opts: { requiredRoles?, requiredPermission?, getRole, permMap?, onDenied? }`                                         | wrapper enforcing RBAC  |
| `withRole(action, opts)`    | `@keyloom/nextjs/rbac`         | `opts: { getUser, adapter, orgId?, requiredRoles?, requiredPermission?, permMap?, onDenied?, rbacEnabled?, config? }` | Response or redirect    |
| `getActiveOrgId()`          | `@keyloom/nextjs/rbac`         | `() => string                                                                                                         | null`                   | reads `__keyloom_org` |
| `setActiveOrgCookie(orgId)` | `@keyloom/nextjs/rbac`         | `(string) => string`                                                                                                  | Set-Cookie header value |
