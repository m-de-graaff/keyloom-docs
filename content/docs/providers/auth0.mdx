...
exports.onExecutePostLogin = async (event, api) => {
  ...
  api.accessToken.setCustomClaim(
    "https://yourdomain.com/permissions",
    user.app_metadata.permissions
  );
};
```
...
<Callout>
  Auth0 requires HTTPS for production applications. Ensure your production
...
<Callout type="warning">
  Password migration requires users to reset passwords or use a gradual
  migration strategy.
</Callout>
