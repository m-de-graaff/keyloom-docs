...
<Callout type="info">
X has transitioned from OAuth 1.0a to OAuth 2.0 with PKCE. This provider uses the modern OAuth 2.0 implementation.
</Callout>
...
async function followUser(accessToken: string, userId: string, targetUserId: string) {
...
<Callout>
X requires HTTPS for production callback URLs. Ensure your production environment has valid SSL certificates.
</Callout>
