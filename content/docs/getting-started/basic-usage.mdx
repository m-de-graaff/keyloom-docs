...
```ts title="keyloom.config.ts (Email + GitHub)"
import { defineKeyloom } from "@keyloom/core";
import { emailPassword } from "@keyloom/providers/email-password"; // if using email/password
...
export default defineKeyloom({
  ...
  providers: [
    emailPassword({
      // password policy, email delivery, etc.
    }),
    github({
