...
<Tabs>
<Tab value="app-router-guard" title="App Router (guard)">
...
<Tab value="middleware" title="Middleware">
...
```ts
// or use createAuthMiddleware with options
import { createAuthMiddleware } from "@keyloom/nextjs/middleware";
import config from "@/keyloom.config";
export default createAuthMiddleware(config, {
...
- CSRF errors on POST: ensure API handler is wired; include the token from `GET /api/auth/csrf` in the `x-keyloom-csrf` header.
