...
<Callout>
  The ResetPasswordForm handles both requesting a reset link and confirming a
  new password. If you already sent the link elsewhere, you can start directly
  at the confirmation stage by entering the token.
</Callout>
...
export const { GET, POST } = createNextHandler(
  (await import("@/keyloom.config")).default
);
```
...
const ResetPasswordForm = dynamic(
  () => import("@keyloom/ui/auth").then((m) => m.ResetPasswordForm),
  { ssr: false }
);
export default function Page() {
  return (
...
- CSRF failures: confirm you invoke Keyloom’s route handler.
